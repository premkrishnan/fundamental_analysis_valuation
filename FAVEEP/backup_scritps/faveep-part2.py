...
def main(url_roic, url_macro):
    ...
    response1 = requests.get(url_roic)
    response2 = requests.get(url_macro)
...
if user_response.lower() == 'yes':                                  # if everything looks okay then go ahead
    ...
    macro = 'https://www.macrotrends.net/stocks/charts/' + ticker.upper() + '/' + company[0].lower() + '/pe-ratio'  # ticker and company name needs to be 'converted' based on 'case'
    
    main(roic, macro)
else:                                                               # if there is a change in the company name
    company_name = user_response.lower()
    roic = 'https://roic.ai/company/' + ticker
    macro = 'https://www.macrotrends.net/stocks/charts/' + ticker.upper() + '/' + company_name + '/pe-ratio'  # ticker and company name needs to be 'converted' based on 'case'
